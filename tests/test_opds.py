...
class TestOPDSCatalog(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sync_db = tempfile.NamedTemporaryFile(delete=False)
        cls.sync_db.close()

        os.environ['LIBRARY_DIR'] = cls.library_dir.name
        os.environ['PAGE_SIZE'] = '1'
        os.environ['KOREADER_SYNC_DB_PATH'] = cls.sync_db.name

        koreader_sync_module = importlib.import_module('koreader_sync')
        cls.koreader_sync = importlib.reload(koreader_sync_module)

        opds_module = importlib.import_module('opds')
        cls.opds = importlib.reload(opds_module)
        ...
        cls.httpd = ThreadedTCPServer(('localhost', 0), cls.server.OPDSHandler)
    ...
    def tearDownClass(cls):
        ...
        if os.path.exists(cls.sync_db.name):
            os.unlink(cls.sync_db.name)
        ...
        importlib.reload(importlib.import_module('koreader_sync'))
        importlib.reload(importlib.import_module('opds'))
    ...
    def test_root_catalog_includes_sections_and_folder(self):
        ...
        self.assertEqual(
            ...
            'application/xml;profile=opds-catalog;kind=navigation',
        )

        ns = {'atom': 'http://www.w3.org/2005/Atom'}
        feed = self._parse_feed(body)
        entry_titles = {
            entry.find('atom:title', ns).text for entry in feed.findall('atom:entry', ns)
        }
    ...
    def test_all_books_feed_paginates_and_lists_books(self):
        ...
        self.assertEqual(
            headers.get('Content-Type'),
            'application/xml;profile=opds-catalog;kind=acquisition',
        )
        ...
        pagination_links = {
            link.get('rel'): link.get('href') for link in feed.findall('atom:link', ns)
        }
    ...
    def test_folder_recent_and_download_endpoints(self):
        ...
        self.assertEqual(
            headers_recent.get('Content-Type'),
            'application/xml;profile=opds-catalog;kind=acquisition',
        )

        recent_feed = self._parse_feed(body_recent)
        recent_titles = [
            entry.find('atom:title', ns).text for entry in recent_feed.findall('atom:entry', ns)
        ]
        self.assertGreaterEqual(len(recent_titles), 2)
        self.assertEqual(recent_titles[0], 'Beta Title')

        status_download, download_headers, download_body = self._get(
            '/download/Subfolder/beta.epub'
        )
